...
function fileProgress(fileObj, bytesLoaded) {
   ...
   try {
      ...
      if (percent === 100) {
         progress.SetStatus("Creating image set...");
...
function fileComplete(fileObj) {
   try {
      //AddImage("thumbnail.aspx?id=" + fileObj.id + "&rnd=" + Math.floor(Math.random()*1000000));
...
function queueComplete() {
   try {
      ...
      progress.SetComplete();
      progress.SetStatus("All images processed.");
      progress.ToggleCancel(false);
   } catch (ex) { this.debugMessage(ex); }
}
