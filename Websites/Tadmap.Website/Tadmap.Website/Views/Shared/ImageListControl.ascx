...
<% 
   ...
   foreach (Tadmap.Website.Models.ImageItem image in ViewData.Model)
   {
%>
<div class="ImageListItem" onclick="window.location = '<%= Url.Action("Index", "Image", new { id = image.Id }) %>'"
   ...
   <img alt="<%= image.Title %>" src="<%= image.IconAvailable ? HttpUtility.HtmlAttributeEncode(image.SquareUrl.OriginalString) : VirtualPathUtility.ToAbsolute("~/App_Themes/Tad/Unavailable.png") %>"
